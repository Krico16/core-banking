...
public class WithdrawFundsUseCase {
    ...
    @Transactional
    public TransactionResult execute(WithdrawCommand command) {
        try {
    ...
    private TransactionResult doExecute(WithdrawCommand command) {
        ...
        List<LedgerEntry> entries = List.of(
                LedgerEntry.debit(journalEntryId.toString(), customerAccount.getId(), customerAccount.getAccountId(), amount),
                LedgerEntry.credit(journalEntryId.toString(), bankCashAccount.getId(), bankCashAccount.getAccountId(), amount)
        );
        ...
        JournalEntry journalEntry = JournalEntry.create(
                ...
                key,
                command.description() != null ? command.description() : "Withdrawal " + currency.code(),
                entries
        );

        customerAccount.debit(amount);
        bankCashAccount.credit(amount);
