...
/** Most payment events go to banking.payment.events, but a few (risk requests,
 * reversal requests) have their own dedicated topic per docs/events/catalog.md. */
const TOPIC_OVERRIDE_BY_EVENT_TYPE: Record<string, string> = {
  PaymentRiskEvaluationRequested: 'PAYMENT_RISK_REQUESTS_TOPIC',
};
...
export class OutboxPublisherWorker {
  ...
  async publishPendingEvents(): Promise<void> {
    const defaultTopic = this.config.get<string>('PAYMENT_EVENTS_TOPIC', 'banking.payment.events');
  ...
  private resolveTopic(eventType: string, defaultTopic: string): string {
    const envKey = TOPIC_OVERRIDE_BY_EVENT_TYPE[eventType];
    if (!envKey) return defaultTopic;
    return this.config.get<string>(envKey, 'banking.payment.risk-requests');
  }
}
